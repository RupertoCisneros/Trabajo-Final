...
import gob.mdmq.remetfu.service.ServicioRetiroAnimales;
...
@CrossOrigin("*")
@RequestMapping("/api")
public class RetiroAnimalRestController {

    @Autowired
    private ServicioRetiroAnimales servicioRetiroAnimales;

    @PostMapping("/crearRetiroAnimal")
    public ResponseEntity<?> crearAtencionAnimal(@RequestBody Map<String, Object> models) {

        try {
            if (servicioRetiroAnimales.crearRetiroAnimales(models)) {
